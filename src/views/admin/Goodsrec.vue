                ...
                <div class="content-top">
                    <div class="text-left padding-25">
                        <div
                            ...
                            style="height:48px;padding:14px 15px;min-width:780px;margin-bottom:15px;"
                        >提示: 请在下方选择要显示的商品; 不选择则将不显示; 点击拖拽更换排序</div>
                        <div
                            class="input-group"
                            style="width:78%;min-width:550px;margin-bottom:13px;"
                        >
                            <input
                                type="text"
                                id="goodsid_text"
                                name="goodsid_text"
                                :title="namestr"
                                class="form-control text bor-rad"
                                readonly
                                v-model="namestr"
                                style="height:30px;"
                            >
                            <div class="input-group-btn">
                                <button
                                    class="btn btn-primary btn-sm select_goods c-blue"
                                    type="button"
                                    @click="changecurindex()"
                                >选择商品</button>
                            </div>
                        </div>
                        <ul style="width:68%;max-width:650px;" id="gsrecsortable" v-show="curindex==0">
                            <li
                                style="display:inline-block;position:relative;margin-right:20px;margin-bottom:8px;"
                                :title="item.name"
                                class="item"
                                data-toggle="tooltip"
                                data-placement="top"
                            ...
                            >
                                <img
                                    ...
                                    :src="'http://entertry.com/upload/'+item.imgpath"
                                    style="width:100px;height:100px;"
                                >
                                <span
                                    class="fa fa-close"
                                    style="position:absolute;top:0;left:103px;color:#aaa"
                                    @click="delitem(item.id)"
                                ></span>
                                <span
                                    style="position:absolute;left:0px;top:73px;height:22px;background-color:black;color:white;display:inline-block;line-height:22px;width:100px;opacity: 0.5;text-align:center;overflow:hidden;font-size:12.8px;"
                                >{{item.name}}</span>
                            </li>
                        ...
                        <div style="margin-top:5px;" >
                            <button
                                v-show="curindex"
                                style="background-color:#45abf5;color:white;border:none;height:25px;width:40px;"
                                @click="changecurindex()"
                            >返回</button>
                        </div>
                    </div>
